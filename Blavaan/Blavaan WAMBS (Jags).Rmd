...
- Installation of R packages `rjags`, `lavaan` and `blavaan`. **This tutorial ony works in Blavaan version `r packageVersion("blavaan")` and Lavaan version `r packageVersion("lavaan")` and was made using R version `r paste0(R.Version()[c("major","minor")], collapse = ".")`** If you obtain any errors, first update your (B)lavaan version.
...
```{r, results='hide', message=FALSE, warning=FALSE, eval=T, cache=TRUE}
...
[expand title="Results" trigclass="noarrow my_button" targclass="my_content" tag="button"]
```{r,  message=FALSE, warning=FALSE, eval=T, cache=TRUE}
summary(fit.bayes)
```
[/expand]
