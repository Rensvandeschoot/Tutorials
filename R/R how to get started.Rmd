...
This tutorial aims to provide the basics of R for beginners. Our detailed instruction will start from the foundations including the installation of R and RStudio, the structure of R screen (I. Who R you?), and loading the data (II. R you ready?).

Next, we introduce basic functions for data exploration and data visualization (III. ExploRe data). Also, we provide how to do statistical analyses such as correlation analysis, multiple linear regression, t-test, and one-way analysis of variance (ANOVA) with easy and intuitive explanation (IV. Analyze and inteRpret data).

The most notable feature is some tips provided in the last section (V. Want moRe?). Based on our ample experience in statistical consultation for methodologists and statisticians who use R, we selected the best advice that will make you smart R users.
...
#### (2) “Can I try this again with R code by typing them in the source pane?”
...
#### (1) Advisors inside of R: Use ?, ??, and the Help tab
...
#### (2) Advisors outside of R: Stack Exchange, Stack Overflow, and Googling tips
